...
def parse(code_path, common_path, protos, _tmp_protos_file):
	...
	for proto in protos:
		...
		ProtoCsharp(code_path, proto).do_server()
...
def protocol_p2p(code_path, mess_name_ids):
	...
	for mess_name_id in mess_name_ids:
		if mess_name_id['mess_name'].startswith('C'):
			mess_name   = 'Req' + mess_name_id['mess_name'][1:]
			var_name    = 'req' + mess_name_id['mess_name'][1:]
